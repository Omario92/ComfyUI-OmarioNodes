...
def extract_model_id(ckpt_name: str) -> str:
    ...
    if path is None:
        path = folder_paths.get_full_path("diffusion_models", ckpt_name)

    if path is None:
        raise FileNotFoundError(
            f"Model file '{ckpt_name}' not found in 'checkpoints' or 'diffusion_models' folders."
        )
...
class GemmaAPITextEncode:
    ...
    @classmethod
    def INPUT_TYPES(cls):
        checkpoints_list = folder_paths.get_filename_list("checkpoints") or []
        diffusion_list = folder_paths.get_filename_list("diffusion_models") or []
        combined_list = checkpoints_list + diffusion_list
    ...
    def encode(self, api_key: str, prompt: str, ckpt_name: str):
        ...
        logger.info(
            "Calling API to encode prompt: %s... with model_id: %s...",
            prompt[:50],
            model_id[:50],
        )
        try:
